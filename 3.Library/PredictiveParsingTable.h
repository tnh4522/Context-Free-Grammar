...
void M::viewProcess(){
  for (Node<Prod>* p =this->list_prod.head; p != NULL; p = p->next){
    for (Node<string>* k = p->data.alpha.head; k != NULL; k = k->next){
      cout << "Xét " << p->data.A << " -> " << k->data << endl;
      LinkedList<string> temp;
      if (k->data == "ε"){
        temp = this->Follow(p->data.A);
        cout << "\tCó follow(" << p->data.A;
      }else{
        temp = this->First(k->data);
        cout << "\tCó first(" << k->data;
      }
      cout << ") = {" << temp << "}"<< endl;
      ...
      Prod prod; prod.A = p->data.A; prod.alpha.AddTail(k->data);
      cout << prod << endl;
    }
  }
}
...
void M::PrintTable() const{
  if (this->list_sigma.Size() > 14){cout << "Bảng quá dài, in ra xấu nên không in !" << endl; return;}
  ...
  for (Node<string>* p = this->list_delta.head; p != NULL; p = p->next){
    ...
    cout << endl;
    string wall_left = "|", sSpam="-"; if (p->next == NULL){wall_left = "¯";sSpam ="¯";}
...
ostream &operator<<(ostream &out, const M &m){
  ...
  if (m.list_sigma.Size() > 14){cout << "Bảng quá dài, in ra xấu nên không in !" << endl; return out;}
...
M::M(const LinkedList<Prod>& list_prod, const LinkedList<string>& delta, const LinkedList<string>& sigma){
  ...
  this->list_sigma = sigma; list_sigma.AddTail("$");
...
void M::FirstLoop(string x, LinkedList<string>& output){
  ...
  if (this->list_sigma.isExist(A) || A == "ε") {                                     //Nếu A ∈ Σ, lấy A
    output.AddTail(A);
    return;
  }
  else{                                                                       //Nếu A ∈ Δ, tìm sản xuất của A
    Prod prod =this->getProd(A);
    for (Node<string>* p = prod.alpha.head; p != NULL; p = p->next){          //Tương ứng với mỗi α, xét tương tự
      string temp = getToken_Lord(p->data).getHead();                          //Lấy ký hiệu đầu của từng α
      if (this->list_sigma.isExist(temp) || temp == "ε"){                            //Kiểm tra α ∈ Σ thì lấy           
        output.AddTail(temp);
      }
      else{
...
LinkedList<string> M::FollowTemp(string A, LinkedList<string>& tracer){
  LinkedList<string> output;
  if (tracer.isExist(A)) return output; //Nếu đã quét qua A rồi thì ngừng quét
...
LinkedList<string> M::FindFollow(string A, LinkedList<string>& tracer){
  tracer.AddTail(A);
  LinkedList<string> output;
  if (A == this->list_prod.getHead().A){                                                //Nếu A là s, thêm $ vào follow(A)
    output.AddTail("$");
  }
  LinkedList<Prod> list_prod = this->getProdContainA(A);     //Lấy ra các sản xuất có chứa A
  for (Node<Prod>* p = list_prod.head; p != NULL; p = p->next){   //Duyệt qua các sản xuất đã tìm được
     for (Node<string>* k = p->data.alpha.head; k != NULL; k = k->next){ //Duyệt qua các α 
        Prod prod; prod.A = p->data.A; prod.alpha.AddTail(k->data);   //Tạo Prod lẻ với mỗi alpha
        string next_A = this->getNextA(A, prod);             //Lấy ký hiệu bên phải A
        if (next_A != "ε"){                                       //Trường hợp A ∈ K -> αAẞ
          if (this->list_sigma.isExist(next_A)){                         //Nếu ẞ ∈ Σ, follow(A) += ẞ
            output.AddTail(next_A);
          }else{
            LinkedList<string> first_temp = this->First(next_A);
            for (Node<string>* t = first_temp.head; t != NULL; t = t->next){
              if (t->data == "ε"){                                  //với ε ∈ first(ẞ), follow(A) += follow(K)
                output += this->FollowTemp(prod.A, tracer);
              }else{                                                //với ε ∉ first(ẞ), follow(A) += first(ẞ)
                output.AddTail(t->data);
              }
            }
          }
        }else{                                                    //Trường hợp A ∈ K -> αA
            output += this->FollowTemp(prod.A, tracer);
        }
     }
  }
